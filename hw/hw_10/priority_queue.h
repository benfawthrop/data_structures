 ...
 private:
  ...
  void percolate_up(int i) {
    ...
    while (i > 0 && *m_heap[(i - 1) / 2] > *m_heap[i]) {
        // while the index exists and the element is greater than its parent,
        // it, and it's parent are swapped
        std::swap(backpointers[m_heap[i]], backpointers[m_heap[(i - 1) / 2]]);
  ...
  void percolate_down(int i) {
    ...
    while (has_left_child(i)) {
        // gets locations of child nodes
        int left_child = 2 * i + 1;
        int right_child = 2 * i + 2;
        int max_index = left_child; // stores the position
        if (right_child < int(m_heap.size()) && *m_heap[right_child] < *m_heap[max_index]) {
            // checks if the right child is larger than the current index
            max_index = right_child;
        }

        if (*m_heap[max_index] < *m_heap[i]) {
            // if we do find a larger child, swap them, and continue to percolate
            std::swap(backpointers[m_heap[i]], backpointers[m_heap[max_index]]);
            std::swap(m_heap[i], m_heap[max_index]);
            i = max_index;
        } else break;
    }
