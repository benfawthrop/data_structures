...
void StudentTests() {
  ...
  PushBack(head, TrainCar::MakeEngine());
  PushBack(head, TrainCar::MakeEngine());
  PushBack(head, TrainCar::MakePassengerCar());
  PushBack(head, TrainCar::MakePassengerCar());
  PushBack(head, TrainCar::MakeDiningCar());
  PushBack(head, TrainCar::MakeSleepingCar());
  PushBack(head, TrainCar::MakeSleepingCar());
  PushBack(head, TrainCar::MakePassengerCar());
  PushBack(head, TrainCar::MakeEngine());
  PushBack(head, TrainCar::MakePassengerCar());
  ...
  PushBack(head, TrainCar::MakePassengerCar());
  PrintTrain(head);
  std::cout << "Testing pop back and pop front" << std::endl;
  delete PopFront(head);
  PrintTrain(head);
  delete PopBack(head);
  PrintTrain(head);
  std::cout << "Testing closest to sleeper" << std::endl << std::endl <<
        ClosestEngineToSleeperCar(head) << std::endl;
