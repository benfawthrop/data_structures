...
int findOrigin(std::vector<std::vector<GRID_STATUS> > &blocked, int x, int y) {
    if (blocked[x][y] == GRID_BLOCKED) {
        return 0;
    } else if (x == 0 && y == 0) {
        return 1;
    } else if (x == 0) {
        return findOrigin(blocked, x, y - 1);
    } else if (y == 0) {
        return findOrigin(blocked, x - 1, y);
    } else {
        return findOrigin(blocked, x - 1, y) + findOrigin(blocked, x, y - 1);
    }
}
...
int main(int argc, char* argv[]) {
    ...
    std::cout << findOrigin(blocked_grid, start_x, start_y) << std::endl;
