...
void print_grid(const std::vector<std::vector<GRID_STATUS> > & blocked_grid,
                unsigned int start_x, unsigned int start_y, std::vector<std::vector<unsigned int> > &path) {
...
int findOrigin(std::vector<std::vector<GRID_STATUS> > &blocked, unsigned int x, unsigned int y,
               std::vector<std::vector<unsigned int> > &coords, std::vector<std::vector<unsigned int> > local_coords,
               int count = 0) {
    if (x == 0 && y == 0) {
        if (count == 0) {
            coords = local_coords;
    ...
    if (blocked[x][y] == GRID_CLEAR) {
        if (count == 0) {
            std::vector<unsigned int> temp;
            temp.push_back(x);
            temp.push_back(y);
            local_coords.push_back(temp);
        }
        if (x > 0) {
            count = findOrigin(blocked, x - 1, y, coords, local_coords, count);
        }
        if (y > 0) {
            count = findOrigin(blocked, x, y - 1, coords, local_coords, count);
        }
        return count;
    } else {
        return count;
...
int main(int argc, char* argv[]) {
    ...
    std::vector<std::vector<unsigned int> > blank;
    std::cout << "Counting paths for " << argv[1] << std::endl;
    std::cout << findOrigin(blocked_grid, start_x, start_y, coords, blank) << std::endl;


    print_grid(blocked_grid, start_x, start_y, coords);
    std::cout << coords.size() << std::endl;
    for (unsigned long int i = 0; i < coords.size(); i++) {
