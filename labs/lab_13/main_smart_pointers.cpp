...
void deleteAllHelper(Balloon* b, std::set<Balloon*> &tmp) {
    if (tmp.find(b) == tmp.end()) {
        tmp.insert(b);
        for (int i = 0; i < b->numRopes(); i++) {
            deleteAllHelper(b->getRope(i), tmp);
        }
    }
}
...
void deleteAll(Balloon* b) {
    std::set<Balloon*> t;
    deleteAllHelper(b, t);
    for (std::set<Balloon*>::iterator it = t.begin(); it != t.end(); ++it) {
        delete *it;
    }
}
...
//    isabelle->print();
    ...
    //
    std::cout << std::endl << std::endl << "Check 2C:" << std::endl;
    dsSharedPtr<Balloon> _georgette(new Balloon("Mr Potato Head"));
    dsSharedPtr<Balloon> _henry(new Balloon("Snoopy"));
    _georgette->addRope(_henry);
    dsSharedPtr<Balloon> temp(new Balloon("Tigger"));
    _georgette->addRope(temp);
    _georgette->print();
    temp->print();

    dsSharedPtr<Balloon> _isabelle(new Balloon("Shrek"));
    _henry->addRope(_isabelle);
    dsSharedPtr<Balloon> temp2(new Balloon("Barney the Purple Dinosaur"));
    _georgette->addRope(temp2);
