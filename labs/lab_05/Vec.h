...
// removes the elements of the vector that are identical to `ele`
template <class T> int remove_matching_elements(Vec<T> &vec_to_use, T ele){
    int count = 0;
    for (unsigned int i = 0; i < vec_to_use.size(); i++){
        if (vec_to_use[i] == ele){
            count++;
            vec_to_use.erase(vec_to_use.begin() + i);
            i--;
        }
    }
    return count;
...
// prints data about our vector
template <class T> void Vec<T>::print() {
    std::cout << "Mem Allocated: " << m_alloc << std::endl;
    std::cout << "Size of the Vector: " << m_size << std::endl;
