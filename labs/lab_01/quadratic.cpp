...
bool find_roots(int a, int b, int c, float &root_pos, float &root_neg) {
  ...
  float sqrt_radical = sqrt(radical); // changed this from radikal to radical  
  // compute the two roots
  // changed the definitions from root_1 and root_2 to root_pos and root_neg for the passing by reference
  root_pos = (-b + sqrt_radical) / float(2*a);
  root_neg = (-b - sqrt_radical) / float(2*a);
  // std::cout << "Your roots at this point in the program are: " << root_pos << " " << root_neg << std::endl;
  return true;
}
