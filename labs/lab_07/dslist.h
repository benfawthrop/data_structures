...
void dslist<T>::push_front(const T& v) {
    Node<T>* node_to_add = new Node<T>(v);
    if (!head_) {
        head_ = tail_ = node_to_add;
    } else {
        Node<T>* old_first = head_;
        head_ = node_to_add;
        node_to_add->next_ = old_first;
        old_first->prev_ = node_to_add;
    }
...
void dslist<T>::pop_front() {
    ...
    Node<T>* node_to_del = head_;
    Node<T>* next_node = node_to_del->next_;
    delete node_to_del;
    head_ = next_node;
    size_--;
}
