...
public:
    ...
    std::pair< iterator, bool > insert(KeyType const& key) {
        ...
        // checks to see if the object is alr in the bucket at the correct space
        ...
        // returns false if the obj already exists
...
private:
    ...
    void resize_table(unsigned int new_size) {
        ...
        std::vector< std::list<KeyType> > temp(new_size);
        m_size = 0;
        for (int i = 0; i < m_table.size(); i++) {
            for (hash_list_itr p = m_table[i].begin(); p != m_table[i].end(); p++){
                unsigned int hash_value = m_hash(*p);
                unsigned int index = hash_value % temp.size();
